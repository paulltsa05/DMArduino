 ...
 */
//replacing with uint16_t for seconf argument because always IDs are used as reference to a node
//list_node_t * list_find(list_t *self, void *val) (original)
list_node_t * list_find(list_t *self, uint16_t val) 
{
  ...
  while ((node = list_iterator_next(it))) {
    if (self->match) {
      if (self->match(val, node->componentID)) {
        list_iterator_destroy(it);
        return node;
      }
    } else {
      if (val == node->componentID) {
        list_iterator_destroy(it);
        return node;
      }
    }
...
void list_node_remove(list_t *self, list_node_t *node) 
{
  node->prev
